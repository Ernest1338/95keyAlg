...
gen_key() {
    ...
    mul_of_seven=$[ 1 + $RANDOM * 99 % 999999 ]
    ...
    for i in {0..6}
    do
        if [ $(( ($num_added_together+($i+1)) % 7 )) -eq 0 ]
        then
            # adding this value to the end of mul_of_seven variable
            mul_of_seven=$(echo $mul_of_seven$(($i+1)))
            break
        fi
    done
...
gen_oem() {
    ...
    mul_of_seven=$[ 1 + $RANDOM * 99 % 99999 ]
